...
def dice_coeff_3label(pred, target, ret_arr = False):
    ...
    # print pred.shape
    if len(pred.shape) == 3:
        return dice_coefficient_numpy(pred[0, ...], target[0, ...]), dice_coefficient_numpy(pred[1, ...], target[1, ...]), dice_coefficient_numpy(pred[2, ...], target[2, ...])
    else:
        dice_lv = []
        dice_myo = []
        dice_rv = []
        for i in range(pred.shape[0]):
            lv, myo, rv = dice_coefficient_numpy(pred[i, 0, ...], target[i, 0, ...]), dice_coefficient_numpy(pred[i, 1, ...], target[i, 1, ...]), dice_coefficient_numpy(pred[i, 2, ...], target[i, 2, ...])
            dice_lv.append(lv)
            dice_myo.append(myo)
            dice_rv.append(rv)
    if ret_arr:
