        ...
        for batch_num,sample in enumerate(cur_dataloader):
            ...
            if epoch % 10 == 0 and args.save_img and False:
                grid_image = make_grid([make_grid(data[0, ...].clone().cpu().data, 1, normalize=True), 
                        to_3d(mask)[0, ...].clone().cpu().data, 
                        to_3d(pred_label)[0, ...].clone().cpu().data],
    ...
    for epoch_num in range(start_epoch, max_epoch):
        ...
        for i_batch in range(1, args.num_eval_iter+1):
            ...
            with amp_cm():
                ...
                stu_tea_dice = dice_calcu(np.asarray(stu_pseudo_label.clone().cpu()), pseudo_label.clone().cpu(), ret_arr=True)
                # print(stu_tea_dice)
                ...
                #                     to_3d(pseudo_label)[lq_idx].clone().cpu().data,
                #                     to_3d(ulb_mask)[lq_idx].clone().cpu().data,
                #                     make_grid(lq_s[0].clone().cpu().data,1,normalize=True),
                #                     to_3d(tmp_lq_pl)[0].clone().cpu().data,
                #                     to_3d(pseudo_label_lq)[0].clone().cpu().data],
                #                     3,padding=2,pad_value=1)
                # writer.add_image("train/img1", lq_sample, iter_num)
                
                lq_dice = dice_calcu(np.asarray(pseudo_label[[lq_idx]].clone().cpu()), ulb_mask[[lq_idx]].clone().cpu())
                ...
                if cur_simple_num > 0:
                    cur_simple_ulb_dice = dice_calcu(np.asarray(pseudo_label[simple_ulb_idx].clone().cpu()), ulb_mask[simple_ulb_idx].clone().cpu())
                    for i in range(n_part):
                ...
                if len(simple_ulb) > 0:
                    simple_ulb_dice = dice_calcu(np.asarray(cor_pl.clone().cpu()), cor_gt.clone().cpu())
                else:
                    simple_ulb_dice = [-1]*n_part
                
                if cur_other_ulb_num > 0:
                    other_ulb_dice = dice_calcu(np.asarray(pseudo_label[other_ulb_idx].clone().cpu()), ulb_mask[other_ulb_idx].clone().cpu())
                    for i in range(n_part):
                        other_ulb_avg_dice[i].update(other_ulb_dice[i])
                ulb_dice = dice_calcu(np.asarray(pseudo_label.clone().cpu()), ulb_mask.clone().cpu())
                for i in range(n_part):
            ...
            p_bar.set_description('iteration %d : loss:%.4f, sup_loss:%.4f, unsup_loss:%.4f, cons_w:%.4f, mask_ratio:%.4f, ulb_dice:%.4f,%.4f,%.4f' 
                                % (iter_num, loss.item(), sup_loss.item(), unsup_loss.item(), consistency_weight, mask.mean(), ulb_dice[0], ulb_dice[1], ulb_dice[2]))
            if iter_num % args.num_eval_iter == 0:
                if iter_num % (args.num_eval_iter*5) == 0 and args.save_img:
                    lb_image = make_grid([make_grid(lb_x_w[0, ...].clone().cpu().data,1,normalize=True), 
                                            to_3d(lb_mask)[0,...].clone().cpu().data, 
                                            to_3d(torch.max(torch.softmax(logits_lb_x_w, dim=1), dim=1)[1])[0, ...].clone().cpu().data],
                                            3,padding=2, pad_value=1)
                    ulb_image = make_grid([make_grid(ulb_x_w[0, ...].clone().cpu().data,1,normalize=True), 
                                            to_3d(ulb_mask)[0,...].clone().cpu().data, 
                                            make_grid(ulb_x_s[0, ...].clone().cpu().data,1,normalize=True), 
                                            to_3d(pseudo_label)[0, ...].clone().cpu().data, 
                                            make_grid(ulb_x_s_ul[0, ...].clone().cpu().data,1,normalize=True), 
                                            to_3d(torch.max(torch.softmax(logits_ulb_x_s_ul, dim=1), dim=1)[1])[0, ...].clone().cpu().data],
