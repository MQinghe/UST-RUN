...
def test(args, model, test_dataloader, epoch, writer, ema=True):
    ...
    for i in range(domain_num):
        ...
        for batch_num,sample in enumerate(cur_dataloader):
            ...
            dice = dice_calcu(np.asarray(pred_label),mask)
            ...
            if epoch % 10 == 0 and args.save_img:
...
def train(args, snapshot_path):
    ...
    for epoch_num in range(start_epoch, max_epoch):
        ...
        for i_batch in range(1, args.num_eval_iter+1):
            ...
            p_bar.set_description('iteration %d : loss:%.3f, sup_loss:%.3f, unsup_loss_ul:%.3f, unsup_loss_lu:%.3f, unsup_loss_s:%.3f, cons_w:%.3f, mask_ratio:%.3f,%.4f,%.4f, dice:%.4f,%.4f,%.4f' 
                                % (iter_num, loss.item(), sup_loss.item(), unsup_loss_ul.item(), unsup_loss_lu.item(), unsup_loss_s.item(), consistency_weight, mask.mean(), ratio_before_ensemble, ratio_after_ensemble, ulb_dice[0], ulb_dice[1], ulb_dice[2]))
